...
int compare(string a,string b){
    ...
    if(neg){
        ...
        if(com==1)return 1;
        if(com==0)return 2;
        if(com==2)return compare(a.substr(a.find('.')+1,a.length()-a.find('.')),b.substr(b.find('.')+1,b.length()-b.find('.')));
    }
...
string calcbase::add(string a,string b,bool xs){
    ...
    int fa=a.find('.');
    int fb=b.find('.');
    if(a.find('-')!=string::npos){k1=1;a.erase(a.begin());}
    if(b.find('-')!=string::npos){k2=1;b.erase(b.begin());}
    ...
    if(xs)for(int i=1;i<=a.length()-c.length();i++)c.insert(c.begin(),'0');
...
string calcbase::sub(string a,string b,bool xs){
    ...
    int w1=0,w2=0,wr,sr,la=a.length(),lb=b.length();
    bool k1=0,k2=0;
    if(*a.begin()=='-'){k1=1;a.erase(a.begin());}
    if(*b.begin()=='-'){k2=1;b.erase(b.begin());}
